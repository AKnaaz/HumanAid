...
const MyVolunteerNeedPost = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://my-eleventh-assignment-server-mauve.vercel.app/myVols?email=${user.email}`, {
      credentials: 'include'
    })
      .then(res => {
        if (!res.ok) {
  ...
  const handleDelete = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://my-eleventh-assignment-server-mauve.vercel.app/myVols/${_id}`, {
          method: "DELETE",
          credentials: 'include'
        })
          .then(res => {
